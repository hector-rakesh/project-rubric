...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def __init__(self, 
        ...
        # 4. Create or get collection with OpenAI embedding function
    ...
    def chunk_text(self, text: str, metadata: Dict[str, Any]) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        if len(text) <= self.chunk_size:
            return [(text, {**metadata, "chunk_index": 0})]
        ...
        while start < len(text):
            ...
            # Try to break at a newline or sentence boundary within the last 20% of the chunk
            if end < len(text):
                search_area = text[end - int(self.chunk_size * 0.2):end]
                last_boundary = max(search_area.rfind('\n'), search_area.rfind('. '))
                if last_boundary != -1:
                    end = (end - int(self.chunk_size * 0.2)) + last_boundary + 1
    ...
    def generate_document_id(self, file_path: Path, metadata: Dict[str, Any]) -> str:
        ...
        idx = str(metadata.get('chunk_index', 0)).zfill(4)
        return f"{mission}_{source}_chunk_{idx}"
        ...
        for i in range(0, len(documents), batch_size):
            ...
            for text, meta in batch:
                ...
                if exists and update_mode == 'update':
                    self.update_document(doc_id, text, meta)
                    stats['updated'] += 1
                else:
                    ids.append(doc_id)
                    texts.append(text)
            ...
            if ids:
                self.collection.add(ids=ids, documents=texts, metadatas=metas, embeddings=embeds)
    ...
    def process_all_text_data(self, base_path: str, update_mode: str = 'skip') -> Dict[str, int]:
        ...
        for file_path in files:
            ...
            batch_stats = self.add_documents_to_collection(chunks, file_path, update_mode=update_mode)
            
            # Update global and mission stats
            stats['files_processed'] += 1
            stats['total_chunks'] += len(chunks)
            stats['documents_added'] += batch_stats['added']
            stats['documents_updated'] += batch_stats['updated']
            stats['documents_skipped'] += batch_stats['skipped']
            
            stats['missions'][mission]['files'] += 1
            stats['missions'][mission]['chunks'] += len(chunks)
            stats['missions'][mission]['added'] += batch_stats['added']
            stats['missions'][mission]['updated'] += batch_stats['updated']
            stats['missions'][mission]['skipped'] += batch_stats['skipped']
